...
import { View ,ActivityIndicator,Text, StyleSheet, Dimensions,Pressable,Image, ScrollView, TouchableOpacity, useWindowDimensions} from 'react-native'
import LottieView from 'lottie-react-native'

const url ="https://dronesapp.azurewebsites.net/user/userOrderDetails"
const CARD_WIDTH = Dimensions.get('window').width*0.45
const AcademyOrders=({navigation})=>{
        ...
        if(data){
            if(data.length>=1){
                return data.map((item)=>{
                    return(
                        <TouchableOpacity key={item.id} onPress={()=>{navigation.navigate('AorderDetails',{
                            itemId:item.id
                        })}}>
                        ...
                        <View >
                            <View style={{flexDirection:'row',alignItems:'center',width:'100%'}}>
                               ...
                               <Text style={{color:'#fff',fontSize:16}}>{item.courseName} </Text>
                               </View>
                            </View>
...
<View style={{flexDirection:'row',margin:30,marginTop:20}}>
    <Text style={{color:'rgb(200 176 215)'}}>Institute</Text>
    <View style={{maxWidth:'99%'}}>
        <Text style={{color:'#fff',fontSize:13}}> : {item.instituteName} </Text>
    </View>
</View>
                            </View>
                            </LinearGradient>
